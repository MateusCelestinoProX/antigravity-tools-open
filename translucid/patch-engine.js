...
const appPath = "/Applications/Antigravity Tools.app";
const macosDir = `${appPath}/Contents/MacOS`;
const binPath = `${macosDir}/antigravity-tools-bin`;
const origPath = `${macosDir}/antigravity-tools-bin.orig`;

if (!fs.existsSync(origPath)) {
  if (fs.existsSync(binPath)) {
    fs.copyFileSync(binPath, origPath);
    console.log("📦 Backup criado:", origPath);
  } else if (fs.existsSync(`${macosDir}/antigravity-tools`)) {
    fs.copyFileSync(`${macosDir}/antigravity-tools`, origPath);
    console.log("📦 Backup criado de antigravity-tools:", origPath);
  }
}
...
// 1. PATCH CSS
const cssOffset = 0x1dff5a3;
const origCssLen = 28677;
const origCss = zlib.brotliDecompressSync(buf.subarray(cssOffset, cssOffset + origCssLen)).toString("utf8");

const customGlassCss = `
/* 🍏 TRANSLUCID SIMPLE GLASS OVERRIDES */
:root, [data-theme], [data-theme="light"], [data-theme="dark"], .light, .dark, body, html {
  color-scheme: dark !important;
  --background: transparent !important;
  --foreground: #e6edf3 !important;
  --card: rgba(18, 24, 38, 0.45) !important;
  --popover: rgba(13, 17, 24, 0.95) !important;
...
aside, nav, header, [class*="sidebar"], [class*="navbar"], [class*="header"] {
  background: rgba(13, 18, 28, 0.45) !important;
  backdrop-filter: blur(25px) !important;
  -webkit-backdrop-filter: blur(25px) !important;
  border-color: rgba(255, 255, 255, 0.08) !important;
}
[class*="bg-slate-"], [class*="bg-zinc-"], [class*="bg-neutral-"], [class*="bg-gray-"], [class*="bg-[#"], [class*="bg-white"], .ant-card, .card, [class*="rounded-"] {
  background-color: rgba(18, 24, 38, 0.45) !important;
  backdrop-filter: blur(15px) !important;
  -webkit-backdrop-filter: blur(15px) !important;
  border-color: rgba(255, 255, 255, 0.08) !important;
  color: #e6edf3 !important;
}
table, tr, th, td, .ant-table, .ant-table-cell {
  background: transparent !important;
  border-color: rgba(255, 255, 255, 0.06) !important;
}
.ant-modal-content, .ant-drawer-content, dialog, [role="dialog"] {
...
const compCss = zlib.brotliCompressSync(Buffer.from(newCss, "utf8"), {
  params: { [zlib.constants.BROTLI_PARAM_QUALITY]: 11 }
});

if (compCss.length > origCssLen) {
  throw new Error("CSS excedeu o tamanho alocado: " + compCss.length + " > " + origCssLen);
}

buf.fill(0, cssOffset, cssOffset + origCssLen);
compCss.copy(buf, cssOffset);
buf.writeBigUInt64LE(BigInt(compCss.length), 0x248b820);
console.log("✅ CSS Brotli injetado:", compCss.length, "bytes");

// 2. PATCH JS MASTER ENGINE
const jsOffset = 0x1b4c7d3;
const origJsLen = 535387;
const origJs = zlib.brotliDecompressSync(buf.subarray(jsOffset, jsOffset + origJsLen)).toString("utf8");

const masterEngineJs = `
;(function() {
  'use strict';
  if (window.__agt_engine_active) return;
  window.__agt_engine_active = true;
  ...
  const KEY_THEME = 'agt-translucid-theme-id';
  const KEY_EFFECT = 'agt-translucid-effect-id';
  const KEY_SPEED = 'agt-translucid-speed';
  const KEY_INTENSITY = 'agt-translucid-intensity';
  const KEY_CUSTOM_MODE = 'agt-translucid-custom-mode';
  const KEY_CUSTOM_P = 'agt-translucid-custom-p';
  const KEY_CUSTOM_S = 'agt-translucid-custom-s';
  const KEY_CUSTOM_A = 'agt-translucid-custom-a';
  ...
  function getActiveColors() {
    if (isCustomMode) {
      return { id: 'custom', name: 'Personalizado', primary: customPrimary, keyword: customSecondary, func: customAccent, accents: [customPrimary, customSecondary, customAccent] };
    }
  ...
  function getOrCreateVisualLayers() {
    ...
    let walkingLayer = document.getElementById('agt-walking-layer');
    if (!walkingLayer) {
      walkingLayer = document.createElement('div');
      walkingLayer.id = 'agt-walking-layer';
      walkingLayer.style.cssText = 'position: fixed; inset: 0; pointer-events: none; z-index: 0; overflow: hidden; mix-blend-mode: screen; transition: opacity 0.3s ease;';
      walkingLayer.innerHTML = '<div id="agt-walking-orb-1" class="agt-walking-orb"></div><div id="agt-walking-orb-2" class="agt-walking-orb"></div>';
      root.insertBefore(walkingLayer, root.firstChild);
    }

    let ambientLayer = document.getElementById('agt-ambient-layer');
    if (!ambientLayer) {
      ambientLayer = document.createElement('div');
      ambientLayer.id = 'agt-ambient-layer';
      ambientLayer.style.cssText = 'position: fixed; inset: 0; pointer-events: none; z-index: 0; overflow: hidden; mix-blend-mode: screen; transition: opacity 0.3s ease;';
      root.insertBefore(ambientLayer, root.firstChild);
    }

    let horizonLayer = document.getElementById('agt-horizon-layer');
    if (!horizonLayer) {
      horizonLayer = document.createElement('div');
      horizonLayer.id = 'agt-horizon-layer';
      horizonLayer.style.cssText = 'position: fixed; inset: 0; pointer-events: none; z-index: 0; overflow: hidden; mix-blend-mode: screen; transition: opacity 0.3s ease;';
      horizonLayer.innerHTML = '<div id="agt-horizon-base-glow" class="agt-horizon-base-glow"></div><div id="agt-horizon-radiant-rays" class="agt-horizon-radiant-rays"></div>';
      root.insertBefore(horizonLayer, root.firstChild);
    }

    let celestialLayer = document.getElementById('agt-celestial-layer');
    if (!celestialLayer) {
      celestialLayer = document.createElement('div');
      celestialLayer.id = 'agt-celestial-layer';
      celestialLayer.style.cssText = 'position: fixed; inset: 0; pointer-events: none; z-index: 0; overflow: hidden; mix-blend-mode: screen; transition: opacity 0.3s ease;';
      celestialLayer.innerHTML = '<div id="agt-celestial-source" class="agt-celestial-source"></div><div id="agt-celestial-rays" class="agt-celestial-rays"></div>';
      root.insertBefore(celestialLayer, root.firstChild);
    }

    let pulseLayer = document.getElementById('agt-pulse-layer');
    if (!pulseLayer) {
      pulseLayer = document.createElement('div');
      pulseLayer.id = 'agt-pulse-layer';
      pulseLayer.style.cssText = 'position: fixed; inset: 0; pointer-events: none; z-index: 0; overflow: hidden; mix-blend-mode: screen; transition: opacity 0.3s ease;';
      pulseLayer.innerHTML = '<div id="agt-pulse-beam-1" class="agt-cyber-beam"></div><div id="agt-pulse-beam-2" class="agt-cyber-beam"></div>';
      root.insertBefore(pulseLayer, root.firstChild);
    }
    ...
    return {
      ...
      orb1: document.getElementById('agt-walking-orb-1'),
      orb2: document.getElementById('agt-walking-orb-2'),
      horizonBaseGlow: document.getElementById('agt-horizon-base-glow'),
      horizonRadiantRays: document.getElementById('agt-horizon-radiant-rays'),
      celestialSource: document.getElementById('agt-celestial-source'),
      celestialRays: document.getElementById('agt-celestial-rays'),
      beam1: document.getElementById('agt-pulse-beam-1'),
      beam2: document.getElementById('agt-pulse-beam-2')
    };
  }
  ...
  function updateVisualLayers() {
    ...
    if (activeEffectId === 'walking-lava') {
      ...
      if (layers.orb1 && layers.orb2) {
        ...
        layers.orb1.style.animation = 'agtOrbWalk1 ' + durationSec + 's cubic-bezier(0.42, 0, 0.58, 1) infinite alternate';
        layers.orb2.style.animation = 'agtOrbWalk2 ' + durationSec + 's cubic-bezier(0.42, 0, 0.58, 1) infinite alternate';
      }
      ...
      if (layers.horizonBaseGlow && layers.horizonRadiantRays) {
        ...
        layers.horizonBaseGlow.style.animation = 'agtHorizonBasePulse ' + (durationSec * 0.7).toFixed(1) + 's ease-in-out infinite alternate';
        ...
        layers.horizonRadiantRays.style.animation = 'agtHorizonRaysPulse ' + (durationSec * 0.85).toFixed(1) + 's ease-in-out infinite alternate';
      }
      ...
      if (layers.celestialSource && layers.celestialRays) {
        ...
        layers.celestialSource.style.animation = 'agtCelestialSourceGlow ' + (durationSec * 0.6).toFixed(1) + 's ease-in-out infinite alternate';
        ...
        layers.celestialRays.style.animation = 'agtCelestialRaysPulse ' + (durationSec * 0.8).toFixed(1) + 's ease-in-out infinite alternate';
      }
      ...
      if (layers.beam1 && layers.beam2) {
        ...
        layers.beam1.style.animation = 'agtCyberPulseTop ' + (durationSec * 0.4).toFixed(1) + 's ease-in-out infinite alternate';
        ...
        layers.beam2.style.animation = 'agtCyberPulseRight ' + (durationSec * 0.5).toFixed(1) + 's ease-in-out infinite alternate';
      }
      ...
      layers.ambientLayer.style.animation = 'agtAuroraWaveMovement ' + (durationSec * 0.75).toFixed(1) + 's ease-in-out infinite alternate';

    } else if (activeEffectId === 'pure-glass') {
      // Clean pure glass
    }

    let styleEl = document.getElementById('agt-dynamic-overrides');
    if (!styleEl) {
      styleEl = document.createElement('style');
      styleEl.id = 'agt-dynamic-overrides';
      document.head.appendChild(styleEl);
    }

    styleEl.innerHTML = [
      '.agt-walking-orb { position: absolute; border-radius: 50%; filter: blur(85px); -webkit-filter: blur(85px); will-change: transform; pointer-events: none; }',
      '#agt-walking-orb-1 { width: 720px; height: 720px; top: -80px; left: -80px; }',
      '#agt-walking-orb-2 { width: 760px; height: 760px; bottom: -100px; right: -100px; }',
      '.agt-cyber-beam { position: absolute; pointer-events: none; border-radius: 999px; }',
      '#agt-pulse-beam-1 { top: 0; left: 0; right: 0; height: 4px; }',
      '#agt-pulse-beam-2 { top: 0; bottom: 0; right: 0; width: 4px; }',
      '.agt-horizon-base-glow { position: absolute; bottom: -40px; left: 50%; transform: translateX(-50%); width: 100vw; height: 480px; filter: blur(65px); -webkit-filter: blur(65px); pointer-events: none; will-change: transform, opacity; }',
      '.agt-horizon-radiant-rays { position: absolute; bottom: 0; left: 0; width: 100vw; height: 100vh; pointer-events: none; filter: blur(40px); -webkit-filter: blur(40px); will-change: opacity, transform; }',
      '.agt-celestial-source { position: absolute; top: -70px; right: -70px; width: 720px; height: 720px; border-radius: 50%; filter: blur(75px); -webkit-filter: blur(75px); pointer-events: none; will-change: opacity, transform; }',
      '.agt-celestial-rays { position: absolute; top: 0; right: 0; width: 100vw; height: 100vh; pointer-events: none; filter: blur(40px); -webkit-filter: blur(40px); will-change: opacity; }',
      '@keyframes agtHorizonBasePulse { 0% { transform: translateX(-50%) scaleY(0.95); opacity: ' + (activeIntensity * 1.0).toFixed(2) + '; } 50% { transform: translateX(-50%) scaleY(1.30); opacity: ' + (activeIntensity * 1.45).toFixed(2) + '; } 100% { transform: translateX(-50%) scaleY(0.95); opacity: ' + (activeIntensity * 1.0).toFixed(2) + '; } }',
      '@keyframes agtHorizonRaysPulse { 0% { opacity: ' + (activeIntensity * 0.70).toFixed(2) + '; transform: scaleY(0.95); } 50% { opacity: ' + (activeIntensity * 1.25).toFixed(2) + '; transform: scaleY(1.18); } 100% { opacity: ' + (activeIntensity * 0.70).toFixed(2) + '; transform: scaleY(0.95); } }',
      '@keyframes agtCelestialSourceGlow { 0% { transform: scale(0.95); opacity: ' + (activeIntensity * 0.9).toFixed(2) + '; } 50% { transform: scale(1.08); opacity: ' + (activeIntensity * 1.35).toFixed(2) + '; } 100% { transform: scale(0.95); opacity: ' + (activeIntensity * 0.9).toFixed(2) + '; } }',
      '@keyframes agtCelestialRaysPulse { 0% { opacity: ' + (activeIntensity * 0.65).toFixed(2) + '; transform: rotate(0deg); } 50% { opacity: ' + (activeIntensity * 1.1).toFixed(2) + '; transform: rotate(3deg); } 100% { opacity: ' + (activeIntensity * 0.65).toFixed(2) + '; transform: rotate(0deg); } }',
      '@keyframes agtOrbWalk1 { 0% { transform: translate3d(0px, 0px, 0) scale(1); } 25% { transform: translate3d(440px, 160px, 0) scale(1.22); } 50% { transform: translate3d(820px, -20px, 0) scale(0.92); } 75% { transform: translate3d(320px, 280px, 0) scale(1.20); } 100% { transform: translate3d(640px, 80px, 0) scale(1.06); } }',
      '@keyframes agtOrbWalk2 { 0% { transform: translate3d(0px, 0px, 0) scale(1); } 25% { transform: translate3d(-460px, -170px, 0) scale(1.22); } 50% { transform: translate3d(-860px, 40px, 0) scale(0.90); } 75% { transform: translate3d(-340px, -300px, 0) scale(1.26); } 100% { transform: translate3d(-660px, -90px, 0) scale(1.03); } }',
      '@keyframes agtCyberPulseTop { 0% { transform: translateX(-60%); opacity: 0.2; } 50% { opacity: 1; } 100% { transform: translateX(60%); opacity: 0.2; } }',
      '@keyframes agtCyberPulseRight { 0% { transform: translateY(-60%); opacity: 0.2; } 50% { opacity: 1; } 100% { transform: translateY(60%); opacity: 0.2; } }',
      '@keyframes agtAuroraWaveMovement { 0% { transform: scaleY(1) translateY(0); filter: hue-rotate(-15deg); } 50% { transform: scaleY(1.3) translateY(20px); filter: hue-rotate(15deg); } 100% { transform: scaleY(1) translateY(0); filter: hue-rotate(-15deg); } }',
      ':root, [data-theme], [data-color-scheme] {',
      '  --primary: ' + colors.primary + ' !important;',
      '  --accent: ' + colors.primary + ' !important;',
      '  --ant-primary-color: ' + colors.primary + ' !important;',
      '  --ant-primary-color-hover: ' + colors.func + ' !important;',
      '  --ring: ' + colors.primary + ' !important;',
      '}',
      '#agt-theme-btn, #agt-dashboard-btn { color: ' + colors.primary + ' !important; }'
  ...
  function updateModalUI() {
    const modal = document.getElementById('agt-theme-modal');
    if (!modal) return;

    const colors = getActiveColors();

    const tabPresets = modal.querySelector('#agt-tab-presets');
    const tabCustom = modal.querySelector('#agt-tab-custom');
    const viewPresets = modal.querySelector('#agt-view-presets');
    const viewCustom = modal.querySelector('#agt-view-custom');
  ...
  function openThemeModal() {
    let modal = document.getElementById('agt-theme-modal');
    if (modal) {
    ...
    modal.id = 'agt-theme-modal';
    modal.style.cssText = 'position: fixed; top: 46px; right: 16px; width: 420px; max-height: 85vh; background: rgba(13, 17, 24, 0.96); backdrop-filter: blur(45px); -webkit-backdrop-filter: blur(45px); border: 1px solid rgba(255, 255, 255, 0.18); border-radius: 16px; box-shadow: 0 30px 90px rgba(0, 0, 0, 0.85); z-index: 2147483647; display: flex; flex-direction: column; overflow: hidden; font-family: -apple-system, BlinkMacSystemFont, sans-serif; color: #e6edf3; animation: themeModalFadeIn 0.18s cubic-bezier(0.16, 1, 0.3, 1);';
    ...
    h += '  <div style="display: flex; align-items: center; gap: 8px; font-weight: 600; font-size: 13px;"><span style="font-size: 16px;">⚡</span><span>Controle de Efeitos & Temas (Antigravity Tools)</span></div>';
    h += '  <button id="close-agt-modal-btn" style="background: transparent; border: none; color: #9198a1; font-size: 16px; cursor: pointer; padding: 4px 8px; border-radius: 4px;">✕</button>';
    h += '</div>';
    ...
    h += '    <div id="agt-effects-container" style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 6px;"></div>';
    h += '  </div>';

    h += '  <div style="padding: 12px 16px; border-bottom: 1px solid rgba(255, 255, 255, 0.08); display: flex; flex-direction: column; gap: 10px;">';
    h += '    <div style="display: flex; align-items: center; justify-content: space-between; gap: 10px;"><span style="font-size: 11px; font-weight: 600; color: #cbd5e1;">Brilho / Glow:</span><div style="display: flex; align-items: center; gap: 8px; flex: 1; justify-content: flex-end;"><input id="agt-glow-slider" type="range" min="0.20" max="0.95" step="0.05" value="' + activeIntensity + '" style="width: 130px; cursor: pointer;"><span id="intensity-val-label" style="font-size: 11px; font-weight: 600; min-width: 32px; text-align: right; color: var(--primary);">' + Math.round(activeIntensity * 100) + '%</span></div></div>';
    ...
    h += '  <div style="padding: 8px 16px; background: rgba(0, 0, 0, 0.3); border-bottom: 1px solid rgba(255, 255, 255, 0.08); display: flex; gap: 6px;"><button id="agt-tab-presets" style="flex: 1; padding: 6px 10px; border-radius: 6px; border: none; font-size: 11.5px; font-weight: 600; cursor: pointer;">🎨 Paletas Presets</button><button id="agt-tab-custom" style="flex: 1; padding: 6px 10px; border-radius: 6px; border: none; font-size: 11.5px; font-weight: 600; cursor: pointer;">✨ Cores Custom</button></div>';

    h += '  <div id="agt-view-presets" style="display: flex; flex-direction: column;"><div id="agt-theme-cards" style="padding: 10px 16px; overflow-y: auto; max-height: 240px; display: flex; flex-direction: column; gap: 6px;"></div></div>';

    h += '  <div id="agt-view-custom" style="display: none; flex-direction: column; padding: 12px 16px 16px 16px; gap: 12px;">';
    h += '    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px;">';
    h += '      <div style="display: flex; flex-direction: column; gap: 4px; background: rgba(255,255,255,0.03); padding: 8px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.06);"><span style="font-size: 10px; color: #94a3b8; font-weight: 600;">Luz Principal</span><input id="agt-cp-p" type="color" value="' + customPrimary + '" style="width: 100%; height: 28px; border: none; cursor: pointer; background: transparent;"></div>';
    h += '      <div style="display: flex; flex-direction: column; gap: 4px; background: rgba(255,255,255,0.03); padding: 8px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.06);"><span style="font-size: 10px; color: #94a3b8; font-weight: 600;">Luz Secundária</span><input id="agt-cp-s" type="color" value="' + customSecondary + '" style="width: 100%; height: 28px; border: none; cursor: pointer; background: transparent;"></div>';
    h += '      <div style="display: flex; flex-direction: column; gap: 4px; background: rgba(255,255,255,0.03); padding: 8px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.06);"><span style="font-size: 10px; color: #94a3b8; font-weight: 600;">Destaque</span><input id="agt-cp-a" type="color" value="' + customAccent + '" style="width: 100%; height: 28px; border: none; cursor: pointer; background: transparent;"></div>';
    h += '    </div>';
    ...
    const effectsContainer = modal.querySelector('#agt-effects-container');
    ...
    const slider = modal.querySelector('#agt-glow-slider');
    ...
    const cpP = modal.querySelector('#agt-cp-p');
    const cpS = modal.querySelector('#agt-cp-s');
    const cpA = modal.querySelector('#agt-cp-a');
    ...
    const themeCardsContainer = modal.querySelector('#agt-theme-cards');
    ...
    modal.querySelector('#close-agt-modal-btn').addEventListener('click', function() { modal.style.display = 'none'; });
    document.addEventListener('click', function(e) {
      if (modal.style.display !== 'none' && !modal.contains(e.target) && !e.target.closest('#agt-theme-btn')) {
  ...
  function mountButtons() {
    ...
    if (topHeader) {
      let group = document.getElementById('agt-btn-group');
      if (!group) {
        group = document.createElement('div');
        group.id = 'agt-btn-group';
        group.style.cssText = 'display: inline-flex !important; align-items: center !important; gap: 4px !important; margin-left: auto !important; margin-right: 12px !important; z-index: 99999 !important;';
        topHeader.appendChild(group);
      }

      if (!document.getElementById('agt-dashboard-btn')) {
        const dashBtn = document.createElement('a');
        dashBtn.id = 'agt-dashboard-btn';
        dashBtn.href = 'http://localhost:3030/';
        dashBtn.target = '_blank';
        dashBtn.title = 'Abrir Control Center (http://localhost:3030/)';
        dashBtn.className = 'agt-native-titlebar-btn';
      ...
      if (!document.getElementById('agt-theme-btn')) {
        const themeBtn = document.createElement('button');
        themeBtn.id = 'agt-theme-btn';
        themeBtn.type = 'button';
        themeBtn.title = 'Personalizar Efeitos & Cores (Cmd+Shift+T)';
        themeBtn.className = 'agt-native-titlebar-btn';
